...
def collect_reads(sublist):
    ...
    for i in range(len(sublist)):
        ...
        for read in fetched:
            ...
            # Enabeling full-length sequencing analysis, like Nanopore, PacBio, etc.
            # For FLS datasets the reference_length-1 subbstitutes template_length.

            else:
                if read.reference_length-1>=size_range[0] and read.reference_length-1<=size_range[1]:
                    if read.mapping_quality>=map_q and read.is_duplicate==False and read.is_qcfail==False:
                        fragment_start = read.reference_start
                        ...
                        num_GC = int(fragment_seq.sum())
                        GC_dict[abs(read.reference_length-1)][num_GC]+=1
